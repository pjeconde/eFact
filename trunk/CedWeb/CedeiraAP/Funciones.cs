...
namespace Cedeira.SV
{
	...
	public class Fun
	{
		...
		public static DataSet GetDataSetFromJanusGridEx(Janus.Windows.GridEX.GridEX JanusGridEx, string tableName)
		{
			...
			{
				foreach(Janus.Windows.GridEX.GridEXRow curRow in JanusRows)
				{
					JanusDataRow = JanusDataTable.NewRow();
					int rowCount = curRow.Cells.Count;
					for(int counter = 0; counter < rowCount; counter++)
					{
						if(isDone == false)
						{
							...
							{
								string auxTexto = "Predeterminado";
								try
								{
									auxTexto = curRow.Table.Columns[q].Caption.ToString();
								}
								catch
								{

								}
								JanusDataTable.Columns.Add(auxTexto);
							}
							isDone = true;
						}
						JanusDataRow[counter] = curRow.Cells[counter].Text;
					}
			...
			else
			{
				...
				{
					JanusDataRow = JanusDataTable.NewRow();
					int rowCount = JanusGridEx.RootTable.Columns.Count;
					int totalVisible = 0;
					int totalNoVisible = 0;
					for(int counter = 0; counter < rowCount; counter++)
					{
						if(isDone == false)
						{
							for(int q = 0; q < rowCount; q++)
							{
								if(JanusGridEx.RootTable.Columns[q].Visible == true)
								{
									...
									try
									{
										auxTexto = JanusGridEx.RootTable.Columns[q].Caption.ToString();
									}
									catch
									{
						...
						if(JanusGridEx.RootTable.Columns[counter].Visible == true)
						{
							JanusDataRow[totalVisible] = JanusGridEx.GetRow(registros).Cells[totalVisible + totalNoVisible].Text;
							totalVisible++;
						}
						else if(JanusGridEx.RootTable.Columns[counter].IsGrouped && JanusGridEx.GetRow(registros).Cells[counter].Text.Equals(String.Empty))
						{
							if(cabecera)
							{
								JanusDataRow[counter] = JanusGridEx.GetRow(registros).GroupValue;
								cabecera = false;
								break;
							}
							else
